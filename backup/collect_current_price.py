...
def get_current_price(ts_code=None):
    ...
    try:
        ...
        shared_data.ths_common_control['期货代码'][0].set_text(ts_code)
        sleep(random.uniform(0.2, 0.5))
        shared_data.ths_common_control['期货代码'][0].type_keys('{ENTER}')
        sleep(random.uniform(0.2, 0.5))
        shared_data.click_position(shared_data.ths_common_control["商品锁"][1])
    except:
        print_context("⚠️ 切换合约失败，继续重试...")

    # 主循环
    while True:
        shared_data.trade_flag()

        # 非交易时间
        if not shared_data.tradeoff:
            if not is_printed:
                print_context(f"[{datetime.now()}] 非交易时间，等待开盘...")
